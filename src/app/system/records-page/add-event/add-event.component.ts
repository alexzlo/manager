...
export class AddEventComponent implements OnInit, OnDestroy {
  ...
  types = [
    ...
    {type: 'outcome', label: 'consumption'}
  ];
  ...
  ngOnInit() {
    this.message = new MassageModel('danger', '');
  }
  ...
  onSubmit(form: NgForm) {
    ...
    const event = new EventModel(
      type, amount, +category, moment().format('DD.MM.YY HH:mm:ss'), description
    );
      ...
      .subscribe((bill: BillModel) => {
        ...
        if (type === 'outcome') {
          if (amount > bill.value) {
            this.showMessage(`You don't have money. You need ${amount - bill.value} `);
            return;
          } else {
